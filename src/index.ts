...
const getZkFetchProof = async (idProtocol: string) => {
    try {
        const proof =  await reclaimClient.zkFetch(
          `https://originx-backend.vercel.app/staking/${idProtocol}`,
          {
            method: "GET",
          },
          {
    ...
    const message = `Hello, this is a signed message from the OriginX Service Manager.`;
    ...
    const tx = await originXServiceManager.respondToApproveTask(
...
const registerOperator = async () => {
    ...
    const operatorDigestHash = await avsDirectory.calculateOperatorAVSRegistrationDigestHash(
        wallet.address, 
        await originXServiceManager.getAddress(), 
        salt, 
        expiry
    );
